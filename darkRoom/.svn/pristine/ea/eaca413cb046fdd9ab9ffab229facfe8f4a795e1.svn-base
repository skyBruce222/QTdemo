...
//    m_pButtonClose->setToolTip(QStringLiteral("关闭"));
    ...
    m_pButtonMax->setIcon(style()->standardPixmap(QStyle::SP_TitleBarMaxButton));
    m_pButtonMin->setIcon(style()->standardPixmap(QStyle::SP_TitleBarMinButton));
    m_pButtonClose->setIcon(style()->standardPixmap(QStyle::SP_TitleBarCloseButton));
    m_pButtonRestore->setIcon(style()->standardPixmap(QStyle::SP_TitleBarNormalButton));
    m_pButtonMax->setFlat(true);
    m_pButtonMin->setFlat(true);
    m_pButtonClose->setFlat(true);
    m_pButtonRestore->setFlat(true);

    m_pIcon->setAutoFillBackground(true);
    m_pTitleContent->setAutoFillBackground(true);
    ...
    mylayout->setContentsMargins(5, 0, 2, 5);
...
void MyTitleBar::setBackgroundColor(int r, int g, int b, bool isTransparent)
{
    ...
    // 重新绘制（调用paintEvent事件）;
    m_pIcon->setStyleSheet("background-color: rgb(" + QString::number(r) + "," + QString::number(g) + "," + QString::number(b) + ");");
    m_pTitleContent->setStyleSheet("background-color: rgb(" + QString::number(r) + "," + QString::number(g) + "," + QString::number(b) + ");");
    update();
}
